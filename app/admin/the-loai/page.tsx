...
export default function AdminGenresPage() {
    ...
    return (
        <div className="container mx-auto px-4 py-6 max-w-5xl">
            <h1 className="text-2xl font-bold text-foreground mb-1">🏷️ Quản lý thể loại</h1>
            <p className="text-sm text-muted mb-6">Master data — Quản lý danh mục thể loại truyện</p>
            ...
            <div className="bg-surface border border-border rounded-xl p-5 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        <label className="block text-xs font-medium text-foreground/90 mb-1">Tên thể loại *</label>
                        <input value={form.name} onChange={e => handleNameChange(e.target.value)}
                            placeholder="VD: Tiên Hiệp"
                            className="form-control" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-foreground/90 mb-1">Slug *</label>
                        <input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })}
                            placeholder="tien-hiep"
                            className="form-control" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-foreground/90 mb-1">Mô tả</label>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-border bg-surface-2">
                                <th className="text-left px-4 py-3 text-xs text-muted font-semibold uppercase">#</th>
                                <th className="text-left px-4 py-3 text-xs text-muted font-semibold uppercase">Tên</th>
                                <th className="text-left px-4 py-3 text-xs text-muted font-semibold uppercase">Slug</th>
                                <th className="text-left px-4 py-3 text-xs text-muted font-semibold uppercase">Mô tả</th>
                                <th className="text-center px-4 py-3 text-xs text-muted font-semibold uppercase">Số truyện</th>
                        ...
                        <tbody className="divide-y divide-border">
                            {genres.map((g, i) => (
                                <tr key={g._id} className="hover:bg-surface-2 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-muted font-mono text-xs">{g.slug}</td>
                                    <td className="px-4 py-3 text-muted truncate max-w-[200px]">{g.description || '—'}</td>
                            ...
                            {genres.length === 0 && (
                                <tr><td colSpan={6} className="text-center py-8 text-muted-2">Chưa có thể loại nào</td></tr>
                            )}
                        </tbody>
                    </table>
                )}
